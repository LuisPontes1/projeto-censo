...
try:
    ...
    if target_sheet in xls.sheet_names:
        ...
        print("--- SEARCHING FOR 'NÃO ALFABETIZADAS' ---")
        mask = df.apply(lambda x: x.astype(str).str.contains('não alfabetizadas', case=False, na=False))
        if mask.any().any():
            idx = mask.any(axis=1).idxmax()
            print(df.iloc[idx:idx+15, :5].to_string())
